...
@Component({
  selector: 'app-report-expired-goods',
  templateUrl: './report-expired-goods.component.html',
  styleUrls: ['./report-expired-goods.component.css']
})
export class ReportExpiredGoodsComponent implements OnInit {
  ...
  exportTable(){
    this.exportService.exportExcel(this.dataSource.filteredData, 'report-goods-expred');
  }

  public loadData() {
    this.isLoadingResults = true;
    this.model.action = 'report-expired-goods';
    ...
    this.dataService.getData(this.model).subscribe(response => {
      if (response.status === 'success') {
        ...
        let tmp: GoodsConsumptionData[] = this.dataFromServer['expired_goods_list'];
